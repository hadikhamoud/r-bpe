...
class RBPETokenizer:
    ...
    def __init__(
        ...
        if clean_data and not cleaned_data_dir:
            logger.warning(
                "cleaned_data_dir was not provided. Cleaned data will not be saved to disk."
            )
    ...
    def prepare(self) -> PreTrainedTokenizerBase:
        ...
        self.reusable_languages_dict, total_reusable_count = (
            self.token_classifier.get_reusable_languages_and_count()
        )
        self.target_language_scripts_ranges = (
            self.token_classifier.get_target_language_scripts_ranges()
        )
        ...
        self.tokenizer = dynamic_tokenizer_class(
            mapping_tokenizer=self.mapping_tokenizer, model_id=self.model_id
        )
